...
"""
function extract_excitations(vector_position::AbstractVector, matrix_separation::AbstractMatrix, J::Function = Jacobian.Standard.J₀; 
    ...
    Methods.Spectral.correlation_to_activity!(tmp, J, fourier_type = fourier_type);
...
Extract correlation function from mean square separation `matrix`, for a polymer with Jacobian `J::Function`. Specify `fourier_type` to switch between different fourier transforms such as DCT or FFT. 
"""
function extract_excitations(matrix_separation::AbstractMatrix, J::Function = Jacobian.Standard.J₀;
        fourier_type = FastFourier.DCT)
