...
function test_homogeneous(; N=512, tol=1e-2)
    ...
    ΔR_numeric  = ActivePolymer.TransformForward.compute_conformation(C_diagonal) |> ActivePolymer.MethodsReal.correlation_to_separation;
    ΔR_analytic = [ActivePolymer.MethodsAnalytic.separation_rouse(s₁, s₂) for s₁=0:N-1, s₂=0:N-1];
...
function test_cosine_1(ϵ, α; N=512, tol=1e-2)
    ...
    ΔR_numeric  = ActivePolymer.TransformForward.compute_conformation(C_diagonal) |> ActivePolymer.MethodsReal.correlation_to_separation;
    ΔR_analytic = [ActivePolymer.MethodsAnalytic.separation_rouse(s₁, s₂, amplitude=ϵ, wavemode=α) for s₁=0:N-1, s₂=0:N-1];
...
function test_cosine_2(ϵ, α; N=512, dN=64, tol=1e-2)
    ...
    ΔR_numeric  = ActivePolymer.TransformForward.compute_conformation(C_diagonal) |> ActivePolymer.MethodsReal.correlation_to_separation;
    ΔR_analytic = [ActivePolymer.MethodsAnalytic.separation_rouse(s₁, s₂, amplitude=ϵ, wavemode=α) - ActivePolymer.MethodsAnalytic.separation_rouse(s₁, s₂) for s₁=0:N-1, s₂=0:N-1];
